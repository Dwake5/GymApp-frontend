...
class SingleWorkoutHistory extends Component {
  ...
  generateExcercises = exercises => {
    return exercises.length
      ? exercises.map((exercise, idx) => (
          <SingleExerciseHistory exercise={exercise} key={"SWH" + idx} />
        ))
      : null;
  };
  ...
  dateFormat = date => {
    ...
    let newDate =
      sliced.slice(6) + "-" + sliced.slice(3, 5) + "-" + sliced.slice(0, 2);
    return newDate;
  };
  ...
  render() {
    ...
    return (
      <>
        <div
          className="container historyNames"
          style={{ display: "flex", justifyContent: "center" }}
        >
          <h3 className="exerciseDates">
            {" "}
            Workout done on {date ? this.dateFormat(date) : null}
          </h3>
          {this.state.showWorkout ? (
            <button className="dropDown" onClick={this.toggleWorkout}>
              {" "}
              <img
                alt=""
        ...
        <div id={this.state.showWorkout ? "workoutCard" : null}>
